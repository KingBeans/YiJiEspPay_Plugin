...
//        return md5($signString.$this->GetValue('secretKey'));
    ...
    try{
      ...
      # 1、检查签名是否一致
        if($this->checkNotifySign($_POST)){

          $order_info = GetOrder($_POST['merchOrderNo']);
          $extra  = array("tradeNo"=>$_POST['merchOrderNo'],"orderNo"=>$_POST['merchOrderNo'],"orderAmount"=>$order_info['total_inc_tax'],"orderCurrency"=>$this->GetValue('Currency'),"orderStatus"=>'');
          ...
          $transaction = GetClass('ISC_TRANSACTION');
            ...
            if($_POST['resultCode'] == 'EXECUTE_SUCCESS'){
                # 3、查看订单回调的订单状态
                switch ($_POST['status']){
                    case 'success':
                ...
                $newTransaction['message'] = json_encode($_POST);

                $transactionId = $transaction->Create($newTransaction);
                $orderInfo = array($_POST['merchOrderNo']=>GetOrder($_POST['merchOrderNo']));
    ...
    */
    public function checkNotifySign($items = array()){
        ...
        if($items['signType'] == 'MD5'){
            ...
            return false;
        }elseif ($items['signType'] == 'RSA'){
            $signValue = base64_decode($sign);
            $cerPath = __DIR__.'/yiji.cer';
            ...
            if($pubkeyid){
                ...
                $verifyResult = openssl_verify($signString, $signValue, $pubkeyid);
                file_put_contents(dirname(__FILE__).'/logs/'.date('Ymd').'log.txt',"\r\n".date('Y-m-d H:i:s ')."  verifyResult:".$verifyResult,FILE_APPEND);
    ...
    */
    public function RedirectToProvider($url,$fields=array()){
        $formFields = '';
        file_put_contents(dirname(__FILE__) . "/logs/signRead.txt","[".date('Y-m-d H:i:s')." 请求参数 ]   ".json_encode($fields),FILE_APPEND);
...
// target="iframe_a" <iframe width="100%"  name="iframe_a" style="border-width:0px;height:700px;">
      ...
      //</iframe>
        $GLOBALS['PAYPAGE'] = '
        <iframe width="100%"  name="iframe_a" style="height:600px;">
                  <script type="text/javascript">
                   alert(window.location.href); target="_blank"
                  </script>
      </iframe>
...
//        $request_data['notifyUrl'] = GetConfig('ShopPathSSL').'/checkout.php?action=gateway_ping&provider='.$this->GetId();
    ...
    */
     ...
     * 处理也不回调新
     */
    function vform($postData){
        try{
            file_put_contents(dirname(__FILE__).'/logs/'.date('Ymd').'log.txt',"\r\n".date('Y-m-d H:i:s ')."异步请求参数:".json_encode($postData),FILE_APPEND);
            # 1、检查签名是否一致
            if($this->checkNotifySign($postData)){

                $order_info = GetOrder($postData['merchOrderNo']);
                $extra  = array("tradeNo"=>$postData['merchOrderNo'],"orderNo"=>$postData['merchOrderNo'],"orderAmount"=>$order_info['total_inc_tax'],"orderCurrency"=>$this->GetValue('Currency'),"orderStatus"=>'');
                $newTransaction = array(
                    'providerid' => $this->GetId(),
                    'transactiondate' => time(),
                    'transactionid' => $this->GetCombinedOrderId(),
                ...
                if($postData['resultCode'] == 'EXECUTE_SUCCESS'){
                    # 3、查看订单回调的订单状态
                    ...
                    $transactionId = $transaction->Create($newTransaction);
                    $orderInfo = array($postData['merchOrderNo']=>GetOrder($postData['merchOrderNo']));
                    foreach ($orderInfo as $orderId => $order) {
                        UpdateOrderStatus($orderId, $order_status);
                    ...
                    EmptyCartAndKillCheckout();
                    exit;
                }elseif ($postData['resultCode'] == 'EXECUTE_FAIL'){
                    $order_status = ORDER_STATUS_DECLINED;
                    $newTransaction['status'] = TRANS_STATUS_FAILED;
                    $extra['orderStatus'] = ORDER_STATUS_DECLINED;
                    $GLOBALS['ISC_CLASS_LOG']->LogSystemError(array('payment', $this->GetName()), GetLang('YiJiPaySuccess'), $extra);
