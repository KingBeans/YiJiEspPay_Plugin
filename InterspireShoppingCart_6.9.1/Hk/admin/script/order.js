var Order = {
	...
	HandleAction: function(orderId, action)
	{
		...
		switch(action) {
			...
			case 'voidTransaction':
				...
				break;
			case 'authorizingAction':
				Order.AuthorizingAction(orderId);
				break;
			//case 'YijipayRefundsAction':
			//	Order.YijipayRefundsAction(orderId);
			//	break;
		}
		return false;
	},
    ...
    AuthorizingAction: function (orderId) {
		var isAccept = 'true';
		var resolveReason;
        if(confirm("此订单有无风险?\n\n确定:已联系客户确认订单无风险。\n取消:订单有风险预授权撤销")){
            isAccept = 'true';
            resolveReason = '已联系客户确认订单无风险';
		}else{
            isAccept = 'false';
            resolveReason = '订单有风险预授权撤销';
		}
		$.get('index.php?ToDo=orderorizing',{orderId:orderId,isAccept:isAccept,resolveReason:resolveReason},function(data){
			console.log(data);
			if(data.status){
                window.location= "";
			}else{
    ...
    /*,YijipayRefundsAction:function(orderId){
		var act,res;
        act = prompt("退款金额?\n\n注意:请始终保持两位小数点，如退款100，请填写100.00");
