...
if ($act == 'refund') {
    # get yjpaycn order 
	$order = isset($_REQUEST['order_sn']) ? get_yjpaycn_order($_REQUEST['order_sn']) : false;
	
	if (!$order || $order['pay_code'] != 'yjpaycn') {
		sys_msg($_LANG['yjpaycn_errors']['order_empty']);
	}
	...
	if ($order['pay_status'] == 2) {
		$smarty->assign('pay_info',array(
			'userId'	=>	$payment['yjpaycn_cfg_partner_id'],
		...
		$smarty->assign('ur_here', $GLOBALS['_LANG']['yjpaycn_refund']['page_title']);
		$smarty->assign('page',$GLOBALS['_LANG']['yjpaycn_refund']);
		$smarty->assign('order',$order);
				
		$smarty->display('yjpaycn_refund.htm');
	} else {
		sys_msg($_LANG['yjpaycn_refund']['empty']);
	}
}
...
if ($act == 'doRefund') {
	...
	if ($order['pay_code'] != 'yjpaycn' && $order['pay_status'] != PS_PAYED) {
		sys_msg($_LANG['yjpaycn_errors']['order_empty']);
	}
	...
	// if (true) {
		order_action($order['order_sn'], $order['order_status'], 
			...
			sprintf($GLOBALS['_LANG']['yjpaycn_refund_success'],$refundMoney,$refundNote)
		);
		
		# if click order cancel button
		if (!empty($_POST['cancel_button'])) {
			yjpaycn_order_status($order['order_id'],OS_CANCELED);
		}
		
		ecs_header("Location: order.php?act=info&order_id=" . $order['order_id'] . "\n");
        exit;
	} else {
		sys_msg(sprintf($_LANG['yjpaycn_refund_fail'],$msg->description));
	}
}
...
if ($act == 'authorize') {
	...
	if ($order['pay_status'] == 2) {
		...
		$smarty->assign('ur_here', $GLOBALS['_LANG']['yjpaycn_authorize']['page_title']);
		$smarty->assign('page',$GLOBALS['_LANG']['yjpaycn_authorize']);
		
		$smarty->assign('order', $order);
		
		$smarty->display('yjpaycn_authorize.htm');
	} else {
		sys_msg($_LANG['yjpaycn_authorize']['empty']);
	}
}
...
if ($act == 'doAuthorize') {
	...
	if ($order['pay_code'] != 'yjpaycn' && $order['pay_status'] != PS_PAYED) {
	// if ($order['pay_code'] != 'yjpaycn' && $order['pay_status'] != PS_PAYING) {
		sys_msg($_LANG['yjpaycn_errors']['order_empty']);
	}
	...
	if ($msg->status == 'success') {
		...
		order_action($order['order_sn'], $order_status, $order['shipping_status'], $pay_status, 
			sprintf($GLOBALS['_LANG']['yjpaycn_authorize_success'],$allowOrder ? 'YES' : 'NO',$authorizeNote)
		);
		
		// pay status/order status modify
		if (!$allowOrder) {
			yjpaycn_order_status($order['order_id'],OS_CANCELED);
			// yjpaycn_pay_status($order['order_id'],PS_UNPAYED);
		}	
		
		ecs_header("Location: order.php?act=info&order_id=" . $order['order_id'] . "\n");
        exit;
	} else {
		sys_msg(sprintf($_LANG['yjpaycn_authorize_fail'],$msg->resultMessage));
	}
}
...
function get_yjpaycn_order($order_sn) {
	$sql = <<<EOF
		...
		WHERE 
			OrderInfo.order_sn = '{$order_sn}' AND Payment.pay_code = 'yjpaycn'
EOF;
...
function yjpaycn_order_status($order_id,$order_status = OS_CANCELED) {
	$sql = <<<EOF
...
function yjpaycn_pay_status($order_id,$pay_status = PS_PAYED)  {
	$sql = <<<EOF
