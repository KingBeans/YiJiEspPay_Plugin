...
{
	# check order info
	$order = empty($_REQUEST['order_sn']) ? false : get_yjpaycn_order(addslashes($_REQUEST['order_sn']));
	// yoko debug
    ...
    if (empty($order))   {
        show_message($_LANG['yjpaycn_errors']['order_empty']);
    }
	...
	if (empty($card['country']) && isset($_LANG['yjpaycn_ccode'][trim($order['country_name'])])) {
		$card['country'] = $_LANG['yjpaycn_ccode'][trim($order['country_name'])];
	}
  ...
  //       if (empty($card['number'])) 		$errors['number'] 			= $_LANG['yjpaycn_errors']['number_empty'];
    	
        if (empty($card['first_name'])) 	$errors['holder'] 			= $_LANG['yjpaycn_errors']['holder_empty'];
        if (empty($card['last_name'])) 		$errors['holder'] 			= $_LANG['yjpaycn_errors']['holder_empty'];
		// if (empty($card['security_code'])) 	$errors['security_code'] 	= $_LANG['yjpaycn_errors']['cvv_empty'];
		
  //       if (empty($card['expired_year']) || empty($card['expired_year'])) {
		// 	$errors['expired_year'] = $_LANG['yjpaycn_errors']['expired_empty'];
		// }
		
        
		// # check bill address information
        if (empty($card['country'])) 		$errors['country'] 		= $_LANG['yjpaycn_errors']['country_empty'];
        if (empty($card['state'])) 			$errors['state'] 		= $_LANG['yjpaycn_errors']['state_empty'];
        if (empty($card['city'])) 			$errors['city'] 		= $_LANG['yjpaycn_errors']['city_empty'];
        if (empty($card['address'])) 		$errors['address'] 		= $_LANG['yjpaycn_errors']['address_empty'];
		
        if (empty($card['post_code'])) 		$errors['post_code'] 	= $_LANG['yjpaycn_errors']['zipcode_empty'];
        if (empty($card['email'])) 			$errors['email'] 	 	= $_LANG['yjpaycn_errors']['email_empty'];
        if (empty($card['phone_number']))	$errors['phone_number'] = $_LANG['yjpaycn_errors']['phone_empty'];
	
		// # check card format information
  //       if (!empty($card['email]']) && !filter_var($card['email'], FILTER_VALIDATE_EMAIL)) {
  //           $errors['email'] = $_LANG['yjpaycn_errors']['email_format'];
  //       }
		
  //       if (!empty($card['security_code']) && !preg_match('/^\d{3}$/', $card['security_code'])) {
  //           $errors['security_code'] = $_LANG['yjpaycn_errors']['cvv_format'];
  //       }
		...
		// 		$card['cardType'] = 'JCB';
		// 	} else {
		// 		$errors['number'] = $_LANG['yjpaycn_errors']['number_invalid'];
		// 	}
		// }
        ...
        if (empty($errors)) {
			...
			// file_put_contents("E:/log/countrys.log", "country_name:".$order['country_name']."card_country:".$card['country']."lang_name:".json_encode($_LANG['yjpaycn_ccode'],true).'languge package:'.$GLOBALS['_CFG']['lang'],FILE_APPEND);
				// exit();
			$esp_order->shipToCountry 	= isset($_LANG['yjpaycn_ccode'][trim($order['country_name'])]) ? 
										  $_LANG['yjpaycn_ccode'][trim($order['country_name'])] : $card['country']; 
			...
			$esp_order->merchantEmail	= $payment['yjpaycn_cfg_merchant_email'];
            $esp_order->merchantName	= $payment['yjpaycn_cfg_merchant_name'];
			$esp_order->webSite			= $GLOBALS['ecs']->get_domain();
			
			$esp_order->ipAddress 		= real_ip();
			
			#if ($payment['yjpaycn_cfg_debug']) {
            ...
            $esp_order->currencyCode 	= $payment['yjpaycn_cfg_currency'];
            // TEST TO BE DONE
            // $esp_order->acquiringType   = "CRDIT";
            $esp_order->acquiringType   = $payment['yjpaycn_cfg_payment_type'];
            ...
            $ecs_items = get_yjpaycn_goods($order['order_id']);
            $esp_items = array();
			...
			#  notify url 
			$ntf_url = $GLOBALS['ecs']->url() . 'yjpaycn_receive.php';
			#  return url
			$ret_url = $GLOBALS['ecs']->url() . 'yjpaycn_return.php';
			# 实例化类 取值
			$esp_api = new esp_api(
    ...
    //         } else {
				...
				// 	sprintf($_LANG['yjpaycn_wait_notify'],$order['order_sn'])
				// );
				
    //             ecs_header("Location:" . return_url('yjpaycn') . "\n");
    //             exit();
    //         }
        }
    }
	
	// file_put_contents("E:/log/pament.log", $payment['yjpaycn_cfg_debug']);
	...
	$expires_month = get_yjpaycn_expires_month();
    $expires_year = get_yjpaycn_expires_year();
...
function get_yjpaycn_order($order_sn)
{
    $sql = <<<EOF
        ...
        WHERE Payment.pay_code = 'yjpaycn' AND order_sn = '{$order_sn}' AND OrderInfo.order_status IN ('0','1','5') AND 		
...
function get_yjpaycn_goods($order_id)
{
    $sql = <<<EOF
		SELECT
...
function get_yjpaycn_expires_month() {
    $expires_month = array();
...
function get_yjpaycn_expires_year() {
    $expires_year = array();
    $today = getdate();
