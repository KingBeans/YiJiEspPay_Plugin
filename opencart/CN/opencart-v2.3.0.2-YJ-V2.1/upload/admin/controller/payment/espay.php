...
class ControllerPaymentEspay extends Controller {
        ...
        if ($espay['status'] == ModelPaymentEspay::STATUS_NEW ||
            ...
            if ($result) {
                if ($result->orderStatus == 'ESP_SUCCESS' ) {
                    $this->model_payment_espay->payed($orderID, $result->rowAddTime);
                    ...
                    $this->model_payment_espay->payedFail($orderID, $result->rowAddTime);
                    $this->model_payment_espay->addOrderHistory($orderID,
                    ...
                    $this->model_payment_espay->authorizing($orderID, $result->rowAddTime);
                    $this->model_payment_espay->addOrderHistory($orderID,
    ...
    public function refund() {
        ...
        if ($result && $result->amountLoc > $result->usableRefundMoney) {
            ...
            $data['currencyCode']      = $result->currency;
