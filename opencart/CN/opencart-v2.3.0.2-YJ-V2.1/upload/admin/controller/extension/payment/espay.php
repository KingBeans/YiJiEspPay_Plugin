...
class ControllerExtensionPaymentEspay extends Controller {
    ...
    public function index() {
        ...
        $data['espay_notify'] = 'http://' . $_SERVER['SERVER_NAME'] . '/catalog/controller/extension/payment/espay_notify.php';
        ...
        $data['breadcrumbs'] = array(
            ...
            array(
                ...
                'href' => $this->url->link('extension/payment/espay', 'token=' . $this->session->data['token'], true)
            )
        );
        ...
        $data['action'] = $this->url->link('extension/payment/espay', 'token=' . $this->session->data['token'], true);
        ...
        $this->response->setOutput($this->load->view('extension/payment/espay', $data));
    }
    ...
    public function order() {
        ...
        $espay = $this->model_extension_payment_espay->query($orderID);
        var_dump($espay);exit;
        if ($espay == false) return '';

        if ($espay['status'] == ModelExtensionPaymentEspay::STATUS_NEW ||
            $espay['status'] == ModelExtensionPaymentEspay::STATUS_AUTHORIZED ||
            $espay['status'] == ModelExtensionPaymentEspay::STATUS_AUTHORIZING
        ) {
            $api = new ESPayApi(
            ...
            var_dump($result);
            if ($result) {
                if ($result->orderStatus == 'ESP_SUCCESS' ) {
                    $this->model_extension_payment_espay->payed($orderID, $result->rowAddTime);
                    $this->model_extension_payment_espay->addOrderHistory($orderID,
                        $this->config->get('espay_status_complete'),
                        '<strong>YJF Pay: </strong>' . sprintf('%.2f',$result->amount)
                    );
                    $espay = $this->model_extension_payment_espay->query($orderID);
                } else if ($result->orderStatus == 'ESP_FAILED') {
                    $this->model_extension_payment_espay->payedFail($orderID, $result->rowAddTime);
                    $this->model_extension_payment_espay->addOrderHistory($orderID,
                        $this->config->get('espay_status_fail'),
                        '<strong>YJF Pay Fail: </strong>' . sprintf('%.2f',$result->amount)
                    );
                    $espay = $this->model_extension_payment_espay->query($orderID);
                } else if ($result->orderStatus == 'ESP_PAREVIEW') {
                    $this->model_extension_payment_espay->authorizing($orderID, $result->rowAddTime);
                    $this->model_extension_payment_espay->addOrderHistory($orderID,
                        $this->config->get('espay_status_authorize'),
                        '<strong>YJF Wait authorize: </strong>' . sprintf('%.2f',$result->amount)
                    );
                    $espay = $this->model_extension_payment_espay->query($orderID);
                }
            }
        ...
        return $this->load->view('extension/payment/espay_order.tpl', $data);
    }
    ...
    public function cancel() {
        ...
        if ($espay && $espay['status'] == ModelExtensionPaymentEspay::STATUS_PAYED) {
            if ($this->request->server['REQUEST_METHOD'] == 'POST') {
                ...
                if ($result->resultCode == 'EXECUTE_SUCCESS') {
                    ...
                    $this->model_extension_payment_espay->cancel($orderID);
                    $this->model_extension_payment_espay->addOrderHistory($orderID, $orderStatus, '<strong>YJF Cancel: </strong>' . $message);

                    $this->response->setOutput($this->load->view('extension/payment/espay_message.tpl', array(
                        'token' => 'success',
            ...
            $this->response->setOutput($this->load->view('extension/payment/espay_cancel.tpl', $data));
        } else {
            $data = array(
                'summary' => $this->language->get('view_cancel_deny')
            );

            $this->response->setOutput($this->load->view('extension/payment/espay_cancel_deny.tpl', $data));
        }
    }
    ...
    public function refund() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateRefund()) {
            ...
            if ($result->resultCode == 'EXECUTE_SUCCESS') {
                $this->model_extension_payment_espay->refund($orderID, floatval($refundMoney));
                $this->model_extension_payment_espay->addOrderHistory($orderID, $orderStatus, '<strong>YJF Refund </strong>' . $refundNote . '(' . $refundMoney . ')');
        ...
        if ($result && $result->amount && $result->ESP_SUCCESS) {
            ...
            $data['currencyCode']      = $result->ESP_SUCCESS;
            $data['enableRefundMoney'] = $result->amount;
            ...
            $this->response->setOutput($this->load->view('extension/payment/espay_refund.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('extension/payment/espay_message.tpl', array(
                'token' => 'danger',
    ...
    public function authorize() {
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ($espay['status'] == ModelExtensionPaymentEspay::STATUS_AUTHORIZING) {
                require_once(DIR_SYSTEM . 'vendor/espay_api.php');
                $api = new ESPayApi(
                    $this->config->get('espay_partner_id'),
                    $this->config->get('espay_certificate_cipher'),
                ...
                if (isset($this->request->post['pass'])) {
                    $result = $api->judgment($orderSN);
                } else {
                    $result = $api->judgment($orderSN, false);
                }

                if ($result->resultCode == 'EXECUTE_SUCCESS') {
                    $this->model_extension_payment_espay->authorized($orderID,
                    ...
                    $this->model_extension_payment_espay->addOrderHistory($orderID, $orderStatus, '<strong>YJF Authorizing :</strong>' . $result->resultMessage);
    ...
    protected function validate() {
        # check permission
        if (!$this->user->hasPermission('modify', 'extension/payment/espay')) {
