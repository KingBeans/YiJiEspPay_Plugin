<?php
    ...
    class ControllerPaymentEspay extends Controller {
        ...
        public function index() {
            ...
            $data['action']         = $this->url->link('payment/espay/submit', '', 'SSL');

            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/espay.tpl')) {
                return $this->load->view($this->config->get('config_template') . '/template/payment/espay.tpl', $data);
            } else {
                return $this->load->view('default/template/payment/espay.tpl', $data);
            }
        }
        ...
        public function submit() {
            ...
            $this->load->model('payment/myespay');
            $data['htmlYJP'] = $this->model_payment_myespay->submitForm();
            echo $data['header'];
            echo $data['htmlYJP'];
            echo $data['footer'];
            exit();     
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
                ...
                if ($success) {
                    ...
                    $this->model_payment_espay->createNew($msg['orderID'], $msg['total'], $msg['order_date']);
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/espay_submit.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/espay_submit.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/payment/espay_submit.tpl', $data));
            }
        }
        ...
        protected function submitPay() {
            ...
            #       $secretKey     = $this->model_setting_setting->getSetting('espay_secret_key');
            ...
            $api = new ESPayApi(
                ...
                $config->get('espay_secret_key'),
                $config->get('espay_debug') == 'debug'
            );

            $url = $config->get('config_url') . 'catalog/controller/payment/espay_receive.php';
