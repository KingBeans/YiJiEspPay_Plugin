<?php
    ...
    class yjfpaycnew extends base {
            ...
            $selection = array (
                ...
                'fields' => array (
                    ...
                    array (
                        ...
                        'field' => zen_draw_pull_down_menu ( 'yjfpay_cardType', $cardType, '-------', 'id="yjfpay_cardType"' . $onFocus )
                    )
        ...
        function  javascript_validation() {
            ...
            $js .= ' if (!checkCardType(yjfpay_card_type)) {' . "\n" . '   error_message = error_message + "' . MODULE_PAYMENT_YJFPAY_TEXT_ERROR_MSG_CARD_TYPE . '";' . "\n" . '      error = 1;' . "\n" . '    }' . "\n";
...
//            $_SESSION['cqnew_payment_method_messages'] = $this->_submitFrame($insert_id);
            ...
            $orderID = substr($resultData['merchOrderNo'], 3);
            $returnParam = '';
...
//                'logisticsFee'        => $order->info['shipping_cost'],
	 ...
	 */
        ...
        function validateCardInfo($additionInfo) {
            ...
            $cardType = $additionInfo ['cardType'];
            ...
            $errorMsg = $this->validateCardType ( $cardNum, $cardType );
            if (! empty ( $errorMsg ) && strlen ( $errorMsg ) > 1) {
                return $errorMsg;
            }
         ...
         * @return String
         */
        function validateCardType($cardNum, $cardTp) {
            $msg = "";
            $cardType = $this->getCardTypeByCardNum ( $cardNum );
            if (empty ( $cardTp ) || strlen ( $cardTp ) < 1) {
...
//            }
        ...
        function func_init_JS() {
            ...
            function checkCardType(cardType) {
                if(cvv == cardType || cardType =="" || cardType.lengh < 1) {
                     return false;
                }else {
                     return true;
                }
