...
//    $sign = array_key_pop_new($_POST, 'sign');
    ...
    if (verifySign($_POST)) {
        # read order status
        $prefix = define(MODULE_PAYMENT_YJFPAYCNEW_ORDER_PREFIX) && MODULE_PAYMENT_YJFPAYCNEW_ORDER_PREFIX != null ? MODULE_PAYMENT_YJFPAYCNEW_ORDER_PREFIX : 'zen';
        $orderID = str_replace($prefix,'',$_POST['merchOrderNo']);
        $status  = strtolower($_POST['status']);

        if ($orderHistory = read_pay_history($db, $orderID)) {
            # process notify status
            if ($status == 'success' && $orderHistory != 3) {
                do_pay_success($db, $orderHistory);
            } else if ($status == 'authorizing') {
                do_pay_authorizing($db, $orderHistory);
            } else if ($status == 'fail'&& $orderHistory != 5) {
    ...
    function do_pay_success($db, $orderHistory) {
        ...
        $statusHistory = array(
            ...
            array('fieldName' => 'comments', 'type' => 'string', 'value' => '(' . $_POST['resultMessage'] . '  Order Id:'.$order_id. '  Total Amount:'.$totalAmount.'  Status:'.$_POST['status'].')'),
        );
    ...
    function do_pay_authorizing($db, $orderHistory) {
        ...
        $statusHistory = array(
            ...
            array('fieldName' => 'date_added', 'type' => 'date', 'value' => date('Y-m-d H:i:s')),
            array('fieldName' => 'comments', 'type' => 'string', 'value' => '(' . $_POST['resultMessage'] . '[' . $_POST['resultCode'] . ']'.'  Order Id:'.$orderID. '  Total Amount:'.$totalAmount.'  Status:'.$_POST['status'].'  authorizingInfo:'.$_POST['authorizingInfo'].')')
        );
    ...
    function do_pay_fail($db, $orderHistory) {
        ...
        $statusHistory = array(
            ...
            array('fieldName' => 'date_added', 'type' => 'date', 'value' => date('Y-m-d H:i:s')),
            array('fieldName' => 'comments', 'type' => 'string', 'value' => '(' . $_POST['resultMessage'] .'[' . $_POST['resultCode'] . ']'. '  Order Id:'.$order_id. '  Total Amount:'.$totalAmount.'  Status:'.$_POST['status'].')')
        );
