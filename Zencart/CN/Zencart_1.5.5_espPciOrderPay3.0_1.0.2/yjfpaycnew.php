<?php
    ...
    function read_pay_history($order_id) {
        $sql = "SELECT order_id,order_no,`status`,`pay_total`,pay_date,pay_status,pay_message,refund_date,refund_total,refund_reason,auth_date,auth_accept,auth_reason,auth_message,add_date,add_data FROM yjfpaycnew_history WHERE order_id = :order_id AND `status` = 0";
        global $db;
