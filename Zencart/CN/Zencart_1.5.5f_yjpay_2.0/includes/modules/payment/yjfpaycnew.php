<?php
    ...
    class yjfpaycnew extends base {
        ...
        function selection() {
            ...
            $cardType [] = array (
                "id" => "",
                "text" => MODULE_PAYMENT_YJFPAY_TEXT_TYPE
            );
            $type_arr = array('Visa','MasterCard',JCB) ;
            ...
            $selection = array (
                ...
                'fields' => array (
                    ...
                    array (
                        'title' => MODULE_PAYMENT_YJFPAY_TEXT_CREDIT_CARD_TYPE,
                        'field' => zen_draw_pull_down_menu ( 'yjfpay_cart_type', $cardType, '-------', 'id="yjfpay_cart_type"' . $onFocus )
                    )
        ...
        function pre_confirmation_check() {
            ...
            // 支付提交网关地址
            $payUrl = trim ( MODULE_PAYMENT_YJFPAY_HANDLER );
            // 密钥文件密码
            $FileKey = trim ( MODULE_PAYMENT_YJFPAY_FILEKEY );
            // 下订单
            $this->confirmation ();
            // 支付数据
            ...
            $options  = array_merge($order_products, $order_detail, $service_option, $post_data);
            ...
            // 进行交易
//            $resultObject = $this->payment_submit ( $payUrl, http_build_query ( $options, '', '&' ) );
            exit ();
        }
        ...
        function  confirmation() {
            ...
            $additionInfo = array (
                ...
                'cardType'=>$this->formatEmpty ( $_POST ['yjfpay_cardType'] ),
            ...
            if ($history->RecordCount() && !$history->EOF) {
                ...
                if ($result->success && $result->status === 'success'){
                    $order_status_history_array = array(
                        ...
                        array('fieldName' => 'date_added', 'type' => 'date', 'value' => date('Y-m-d H:i:s')),
                        array('fieldName' => 'comments', 'type' => 'string', '('.MODULE_PAYMENT_YJFPAYCNEW_STATUS_6.')')
                    );
                    $update_history_array = array(
                        array('fieldName' => 'pay_status', 'type' => 'integer', 'value' => MODULE_PAYMENT_YJFPAYCNEW_PAYMENT_STATUS_ID),
                        array('fieldName' => 'pay_date', 'type' => 'date', 'value' => date('Y-m-d H:i:s')),
                        array('fieldName' => 'pay_message', 'type' => 'string', 'value' => '(' . $result->resultMessage . '  Order Id:'.$orderID.'  Status:'.$result->status.')'),
                    ...
                    $messageStack->add_session($result->resultMessage, 'fail');
                }
            }
...
EOF;
        ...
        protected function _optionPost(){
            ...
            $data['cardNo']  = $this->signPublicKey($cardNo);
            $data['cvv']  = $this->signPublicKey($cvv);
        ...
        function signPublicKey($data){
            $public_key_path =  __DIR__.'/'.MODULE_PAYMENT_YJFPAY_MERNO.'.pem';
...
//                'logisticsFee'        => $order->info['shipping_cost'],
         ...
         * @return string
         */
        function payment_submit($formalUrl, $data) {

            // 发送支付请求
            $resultJson = $this->send_gateway ( $formalUrl, $data );
        ...
        function send_gateway($payUrl, $data) {
            ...
            if (function_exists ( 'curl_init' ) && function_exists ( 'curl_exec' )) {
                $resultXml = $this->curl_post ( $payUrl, $data ); // crul请求
            } else {
                $resultXml = $this->http_post ( $payUrl, $data ); // 普通http请求
            }
            return $resultXml;
        }
	 ...
	 */
        ...
        function validateCardType($cardNum) {
            $msg = "";
            $allowType = MODULE_PAYMENT_YJFPAY_CARD_TYPE;

            if ($allowType == "--none--") {
                return MODULE_PAYMENT_YJFPAY_TEXT_ERROR_MSG_NONE_CARD;
            }
            $cardType = $this->getCardTypeByCardNum ( $cardNum );
            if (empty ( $cardType ) || strlen ( $cardType ) < 1 || ! strstr ( $allowType, $cardType )) {
                $msg = MODULE_PAYMENT_YJFPAY_TEXT_ERROR_MSG_CARD_TYPE;
                if (! empty ( $allowType ) && strlen ( $allowType ) > 1) {
                    $msg .= MODULE_PAYMENT_YJFPAY_TEXT_ERROR_MSG_CARD_ALLOW . MODULE_PAYMENT_YJFPAY_CARD_TYPE . ' !\n';
                }
            }
            return $msg;
        }
