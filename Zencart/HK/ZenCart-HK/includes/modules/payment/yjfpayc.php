<?php
    ...
    class yjfpayc extends base {
        ...
        function update_status() {
            ...
            if ($this->enabled && (int)MODULE_PAYMENT_YJFPAYC_ZONE > 0 && isset($order->billing['country']['id'])) {
                $check_flag = false;
                $check      = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_YJFPAYC_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
                while (!$check->EOF) {
        ...
        function  check() {
            ...
            if (!isset($this->_check)) {
                $check_query  = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_YJFPAYC_STATUS'");
        ...
        function after_process() {
            ...
            $order_update_array = array(
                array('fieldName' => 'orders_status', 'type' => 'integer', 'value' => MODULE_PAYMENT_YJFPAYC_SUBMIT_STATUS_ID)
            );
        ...
        function  install() {
            ...
            if (defined('MODULE_PAYMENT_YJFPAYC_STATUS')) {
                $messageStack->add_session('YjfPay module already installed.', 'error');
                zen_redirect(zen_href_link(FILENAME_MODULES, 'set=payment&module=yjfpayc', 'NONSSL'));
                return 'failed';
            }

            if (!defined('MODULE_PAYMENT_YJFPAYC_TEXT_TITLE')) {
                include(DIR_FS_CATALOG_LANGUAGES . $_SESSION['language'] . '/modules/payment/yjfpayc.php');
            }

            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_STATUS_TITLE . "', 'MODULE_PAYMENT_YJFPAYC_STATUS', 'True', '" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_STATUS_DESCRIPTION . "', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'),', now())");
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_MERCHANT_NAME_TITLE . "', 'MODULE_PAYMENT_YJFPAYC_MERCHANT_NAME', '" . '' . "', '" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_MERCHANT_NAME_DESCRIPTION . "', '6', '2', now())");
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_MERCHANT_EMAIL_TITLE . "', 'MODULE_PAYMENT_YJFPAYC_MERCHANT_EMAIL', '" . '' . "', '" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_MERCHANT_EMAIL_DESCRIPTION . "', '6', '3', now())");
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_PARTNER_ID_TITLE . "', 'MODULE_PAYMENT_YJFPAYC_PARTNER_ID', '" . '' . "', '" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_PARTNER_ID_DESCRIPTION . "', '6', '4', now())");
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_SECRET_KEY_TITLE . "', 'MODULE_PAYMENT_YJFPAYC_SECRET_KEY', '', '" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_SECRET_KEY_DESCRIPTION . "', '6', '5', now())");
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function,date_added) values ('" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_ACQUIRING_TYPE_TITLE . "', 'MODULE_PAYMENT_YJFPAYC_ACQUIRING_TYPE', '', '" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_ACQUIRING_TYPE_DESCRIPTION . "', '6', '1', 'zen_cfg_select_option(array(\'CRDIT\', \'YANDEX\'),', now())");
            ...
            $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_ORDER_TITLE . "', 'MODULE_PAYMENT_YJFPAYC_SORT_ORDER', '0', '" . MODULE_PAYMENT_YJFPAYC_CONFIGURATION_ORDER_DESCRIPTION . "', '8', '8', now())");
...
EOF;
        ...
        function keys() {
            return array(
                'MODULE_PAYMENT_YJFPAYC_STATUS',
                'MODULE_PAYMENT_YJFPAYC_MERCHANT_NAME',
                'MODULE_PAYMENT_YJFPAYC_MERCHANT_EMAIL',
                'MODULE_PAYMENT_YJFPAYC_PARTNER_ID',
                'MODULE_PAYMENT_YJFPAYC_SECRET_KEY',
                'MODULE_PAYMENT_YJFPAYC_ACQUIRING_TYPE',
                'MODULE_PAYMENT_YJFPAYC_SUBMIT_STATUS_ID',
        ...
        function admin_notification($order_id) {
            ...
            if ($history->EOF) {
                return '<td>' . MODULE_PAYMENT_YJFPAYC_ADMIN_PAY_EXCEPTION . '</td>';
            }

            $output = '';
            if (file_exists(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/yjfpayc/admin_notification.php')) {
                include(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/yjfpayc/admin_notification.php');
            }
        ...
        function _doRefund($orderID) {
            ...
            if ($history->RecordCount() && !$history->EOF) {
                ...
                if ($refundAmount == false) {
                    $messageStack->add_session(MODULE_PAYMENT_YJFPAYC_ENTRY_REFUND_AMOUNT_TEXT, 'fail');
                    return true;
                }
                ...
                if (isset($_POST['cancel'])) {
                    ...
                    if ($_POST['refundAmount'] > $history->fields['pay_total']) {
                        $messageStack->add_session(MODULE_PAYMENT_YJFPAYC_ENTRY_REFUND_AMOUNT_MORE_TEXT, 'fail');
                        return true;
                    }
                ...
                if ($result->success) {
                    ...
                    if (isset($_POST['cancel']) || $refundAmount == $history['pay_total']) {
                        $order_status_history_array = array(
                            ...
                            array('fieldName' => 'comments', 'type' => 'string', MODULE_PAYMENT_YJFPAYC_STATUS_5)
                        );

                        $update_order_array = array(
                            array('fieldName' => 'orders_status', 'type' => 'int', 'value' => MODULE_PAYMENT_YJFPAYC_FAIL_STATUS_ID)
                        );
                        ...
                        $order_status_history_array = array(
                            array('fieldName' => 'orders_id', 'type' => 'integer', 'value' => $orderID),
                            array('fieldName' => 'orders_status_id', 'type' => 'integer', 'value' => MODULE_PAYMENT_YJFPAYC_SUBMIT_STATUS_ID),
                            array('fieldName' => 'date_added', 'type' => 'date', 'value' => date('Y-m-d H:i:s')),
                            array('fieldName' => 'comments', 'type' => 'string', MODULE_PAYMENT_YJFPAYC_STATUS_4)
                        );
                    }
        ...
        protected function _execute($params) {
            # declare url var
            $url  = MODULE_PAYMENT_YJFPAYC_GATEWAY_URL ? YJFPAYC_PRODUCT_URL : YJFPAYC_DEBUG_URL;
            $curl = curl_init();
...
EOF;
        ...
        protected function _optionService($order, $total, $orderID) {
            return array(
                'userId'        => MODULE_PAYMENT_YJFPAYC_PARTNER_ID,
        ...
        protected function _optionOrderDetail($order) {
            ...
            $detail = array(
                ...
                'merchantEmail'       => MODULE_PAYMENT_YJFPAYC_MERCHANT_NAME,
