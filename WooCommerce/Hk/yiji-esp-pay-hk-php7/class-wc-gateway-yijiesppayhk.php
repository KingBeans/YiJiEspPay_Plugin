...
//          $request_data['notifyUrl'] = WC()->api_request_url( 'WC_YiJiEspPayByHk' );
            $request_data['notifyUrl'] = site_url("?wc-api=WC_YiJiEspPayByHk");
          // 产品信息
        ...
        */
        public function check_request_sign(array $items ){
          ...
          unset($items['sign']);
            unset($items['wc-api']);
            unset($items['woocommerce-login-nonce']);
            unset($items['_wpnonce']);
            ...
            if($_REQUEST['resultCode'] == 'EXECUTE_SUCCESS'){
              ...
              switch ($_REQUEST['status']){
                ...
                $order->reduce_order_stock();
                    file_put_contents(dirname(__FILE__).'/logs/'.date('Ymd').'log.txt',"update_static:".$update_static." [NOTIFY_REQUEST_ORDER_BY_".$_REQUEST['merchOrderNo']." requset_time:".date('Y-m-d H:i:s')." request_ip:".$this->get_real_ip()." ](付款成功(success)):\n".json_encode($_REQUEST)."\n\n",FILE_APPEND);
                break;
                case 'fail':
                ...
                case 'processing':
                $update_static = $order->update_status('processing', __($_REQUEST['status'],$this->id));
                    file_put_contents(dirname(__FILE__).'/logs/'.date('Ymd').'log.txt',"update_static:".$update_static." [NOTIFY_REQUEST_ORDER_BY_".$_REQUEST['merchOrderNo']." requset_time:".date('Y-m-d H:i:s')." request_ip:".$this->get_real_ip()." ](付款成功(processing)):\n".json_encode($_REQUEST)."\n\n",FILE_APPEND);
                break;
              }
