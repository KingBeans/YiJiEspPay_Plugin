...
//            var_dump(WC()->api_request_url( 'WC_YiJiEspPayByCn' ));
            global $woocommerce;
...
<?php
        ...
        public function check_request_sign(array $items ){
            ...
            foreach ($items as $key => $value) {
                if('sign'===$key || 'woocommerce-login-nonce'===$key || '_wpnonce'===$key){
                    continue;
                }
...
//                            $woocommerce->cart->empty_cart();
                ...
                file_put_contents(dirname(__FILE__).'/logs/'.date('Ymd').'read.txt',"[ REQUEST_".$_REQUEST['merchOrderNo']." ]:\n".json_encode($_REQUEST)."\n\n",FILE_APPEND);
                exit("success");
            }else{
