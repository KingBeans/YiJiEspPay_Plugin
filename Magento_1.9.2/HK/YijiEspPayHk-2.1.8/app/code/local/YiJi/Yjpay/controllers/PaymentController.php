...
class YiJi_Yjpay_PaymentController extends Mage_Core_Controller_Front_Action
{
    ...
    public function notifyAction()
    {
        ...
        } else if ($this->getRequest()->isGet())
        {
            ...
            // file_put_contents("E:/log/notify_info/hkinfo1.log", "postJson:".$postJson,FILE_APPEND);
		...
		$mysign = $yjpay->sign($prestr.$security_code);
		//Mage::log("异步请求结果-签名：".$prestr, null,'log.log');

		$service = $postData['service'];
		if ( $mysign == $postData["sign"])  {
            if ($service === 'cardAcquiringCashierPay' OR $service === 'cardAcquiringPresaleResult' OR $service === 'cardAcquiringRefund') {
                ...
                // $postDataLow = strtolower($postData['status']);
                ...
                if($postData['status']=="SUCCESS" || $postData['status'] == "success") { //交易成功
                    ...
                    echo "success";
                    return;
                }elseif($postData['status']=="FAILURE" || $postData['status'] == "fail" || $postData['status'] == "failure"){ ////交易失败，pending->>pending_payment
	...
	*/
	public function authorizationAction(){
		$_orderNo = filter_input(INPUT_GET,'orderNo');
		$action = filter_input(INPUT_GET,'isAccept');
        ...
        $cancelReason = filter_input(INPUT_GET,'re')?filter_input(INPUT_GET,'re'):'agree';
        $isAccept = null;
    	if ($action === "confirm"){
            $isAccept = 'true';
        ...
        if($_api_result){
            ...
            if($arr['resultCode']=='EXECUTE_SUCCESS'){
                if($isAccept=='true'){
                    //$order->addStatusToHistory($order->getStatus(),Mage::helper('yjpay')->__('Pre authorization operation success,waiting for the receipt!'),true);
    ...
    {
    	$_orderNo = filter_input(INPUT_GET,'orderNo');
        $reReason = filter_input(INPUT_GET,'re')?filter_input(INPUT_GET,'re'):'agree';
    	$order_mode = Mage::getModel('sales/order');
    	$order = $order_mode->load(base64_decode($_orderNo));
    ...
    public function returnAction(){
        ...
        # 验签
        $prees = filter_input_array(INPUT_GET);
        $sign = $prees['sign'];
        unset($prees['sign']);
        if( ! $standard->signParaArray($prees) == $sign ){
            Mage::log(filter_input(INPUT_GET,'merchOrderNo')."返回的请求：".json_encode(filter_input_array(INPUT_GET)), null,'Yjpay'.date('Y-m-d').'.log');
            exit('sign is error');
        }

        # 验证订单状态是否为 processing
        if(filter_input(INPUT_GET,'status') == 'processing'){
            $order_mode = Mage::getModel('sales/order');
            $order = $order_mode->load(base64_decode(filter_input(INPUT_GET,'merchOrderNo')));
            ## 冻结订单，等待异步处理
            $order->setState(Mage_Sales_Model_Order::STATE_HOLDED);
        }
        $orderNo = $this->getOrder()->getRealOrderId();
        $resultCode = filter_input(INPUT_GET,'resultCode');
        $success = filter_input(INPUT_GET,'success');
        $resultMessage = filter_input(INPUT_GET,'resultMessage');
        ...
        if($success === 'false'){
            if($style === "redirect"){
                ...
                $showMesaage ='you order # '.$orderNo.' ,<br>error message:'.$resultMessage.
