...
class YiJi_Yjpay_Model_Payment extends Mage_Payment_Model_Method_Abstract
{
    ...
    {
        // $return_url = Mage::getUrl('yjpay/payment/reture', array('_secure' => true));
        // file_put_contents("E:/log/returnurlold.log",$return_url,FILE_APPEND);
        return Mage::getUrl('yjpay/payment/return');
    }
    /**
    ...
    {
        return Mage::getUrl('yjpay/payment/notify/');
    }
    ...
    public function getFormFields()
    {
        ...
        if(!$session->getLastRealOrderId()){
            exit('Please log in and resubmit the order');
        }
        ...
        $attachDetails = array (
            ...
            'shiptoEmail' => $isvirtual?'34930948@qq.com':($shippingaddress->getData('email')?$shippingaddress->getData('email'):$billingaddress->getData('email')),//收货人邮箱
        ...
        $parameter = array(
            //基本参数
            'service'          =>'cardAcquiringCashierPay',   //服务代码
            ...
            'notifyUrl'       =>$noty_url,
            'version'	=> 	'1.0',
            'exVersion'			  =>'MageWeb-1.0.1',
            'signType'	=>	'MD5',
            //业务参数
            'currency' => Mage::app()->getStore()->getCurrentCurrencyCode(), //原始订单币种
            'orderAmount' => $this->chageBaseConvert2SessionConvert($payment->getBaseAmountOrdered()),//原始订单金额
            ...
            'attachDetails' => $attachDetails,//订单扩展信息
            'remark' => '备注001',//备注
            'goodsInfoOrders'   =>$this->getWkGoodsInfoListByOrderList($sales_order->getAllItems()),//货物信息
    ...
    public function signParaArray($parameter_array){
        ...
        $prestr = trim($arg,'&');
        Mage::log("请求待签名字符串：".$prestr.$secretKey, null,'Yjpay'.date('Y-m-d').'.log');
