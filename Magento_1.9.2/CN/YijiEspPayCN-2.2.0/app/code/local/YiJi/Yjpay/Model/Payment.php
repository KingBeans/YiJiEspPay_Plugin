...
class YiJi_Yjpay_Model_Payment extends Mage_Payment_Model_Method_Abstract
{
    ...
    //验签相关数据
    protected $CER_TEST = 'yiji.snet.cer';
    protected $CER_ONLINE = 'yiji.online.cer';
    ...
    public function getFormFields()
    {
		...
		//基本参数
			...
			//业务参数
                ...
                'language' => $this->getConfigData('payment_language')
        );
	...
	public function sign($prestr,$signType = null) {
//        $mysign = md5($prestr);
        $mysign = '0';
        if ($signType === 'RSA'){
            if (strpos($this->getConfigData('pay_gateway'),'openapi.yijifu.net')){
                $pfxPath = MAGENTO_ROOT.'/snet_'.$this->getConfigData('merchant_partnerId').'.pfx';
            }else{
                $pfxPath = MAGENTO_ROOT.'/'.$this->getConfigData('merchant_partnerId').'.pfx';
            }

            $pkcs12 = file_get_contents($pfxPath);
            $keyPass = $this->getConfigData('secretKey');//密码
            ...
            if (openssl_pkcs12_read($pkcs12, $certs, $keyPass)) {
                ...
                if (openssl_sign($prestr, $signedMsg, $privateKey)) {
                    $mysign = base64_encode($signedMsg);
                } else {
                    $mysign = '0';
                }
            }
	...
	public function signParaArray($parameter_array){		
		...
		Mage::log("请求待签名字符串：".$prestr, null,'debug1.log');

		return $this->sign($prestr,$parameter['signType']);
	}
	...
	*/
     ...
     * @return bool|int
     */
    function doVerify($fields){
        if(isset($fields['sign']) && isset($fields['signType'])){
            $signValue = $fields['sign'];
            switch($fields['signType']){
                case "MD5":
                case "RSA":
                    $strDate = $this::parBuild($fields);
                    $cerName = $this->chooseCerFile();
                    $filePath = MAGENTO_ROOT.'/'.$cerName;
                default:
            }
            return $this::verifyByCER($strDate,$signValue,$filePath);
        }else{
            return false;
        }
    }
    function parBuild (array $fields){
        ksort($fields);
        $waitingForSign = '';
        ...
        //echo "<br>".$waitingForSign."<br>";
        return $waitingForSign;
    }

    function verifyByCER($data,$signature,$cerPath){
    ...
    function chooseCerFile(){
        if (strpos($this->getConfigData('pay_gateway'),'openapi.yijifu.net')){
            $cerName = $this->CER_TEST;
        }else{
            $cerName = $this->CER_ONLINE;
        }
        return $cerName;
    }
